...
function SphereCursor( camera, lineThickness, sphereRadius ) {
	var axis, geometry, edges, material, cursor, planarPosition, hidden, updateCallback,
		pointOnSphere, sphericalCoords;
	
	geometry = new THREE.CircleGeometry( lineThickness, 16 );
	...
	window.addEventListener( 'pointermove', function() {
		...
		moveTo( SPHEREHELPER.normalizeAndCenterPoint(
	...
	function update() {
		pointOnSphere = SPHEREHELPER.projectPointOntoSphere(
			planarPosition, sphereRadius - lineThickness - .1,
			SPHEREHELPER.getUnprojectionMatrix( camera )
		);
		...
		TEXTPRINTING.output( "cursorTheta: "
			+ ( sphericalCoords.theta * 180 / Math.PI ).toFixed( 3 ), 5 );
		TEXTPRINTING.output( "cursorPhi: "
			+ ( sphericalCoords.phi * 180 / Math.PI ).toFixed( 3 ), 6 );
	}
